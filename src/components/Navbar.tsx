  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? "bg-card/95 backdrop-blur-md shadow-[var(--shadow-soft)]" : "bg-transparent"
      }`}
    >
      {/* Match sections: horizontal padding wraps container so logo aligns with section content on laptop */}
          ...
          <div className="flex items-center justify-between h-16 sm:h-20">
            <a href="#home" className="flex items-center gap-3 min-w-0">
              ...
              <div className="flex flex-col min-w-0">
                <span className="font-display text-xl sm:text-2xl font-bold text-foreground truncate">
                  Lingan Cream House
                </span>
                <span className="text-xs text-muted-foreground font-body tracking-wider">Since 1970</span>
              </div>
            </a>
      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            <div className="px-4 sm:px-6 lg:px-8">
              <div className="container-narrow mx-auto py-4 space-y-3">
                {navLinks.map((link) => (
                  <a
                    key={link.href}
                    href={link.href}
